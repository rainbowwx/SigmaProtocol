...
// Y1 = w1*G, Y2 = w2*G, Y3 = (w1w2)*G, the required multiplicative relation can
// be proven by observing that the proof goal is equivalent to Y1 = w1*G,
// Y2=w2*G and Y3 = w2*Y1
...
class DiffieHellmanTripleCommonInput : public SigmaProtocolCommonInput {
 public:
  DiffieHellmanTripleCommonInput(const EC_GROUP* Group, const EC_POINT* G,
                                 const EC_POINT* Y1, const EC_POINT* Y2,
                                 const EC_POINT* Y3,
...
class DiffieHellmanTripleProverInput : public SigmaProtocolProverInput {
 public:
  DiffieHellmanTripleProverInput(const BIGNUM* w1, const BIGNUM* w2)
      : w1(w1), w2(w2) {}
...
class DiffieHellmanTripleMessage : public SigmaProtocolResponseMessage {
 public:
  explicit DiffieHellmanTripleMessage(const EC_GROUP* Group)
      : Group(Group),
  ...
  DiffieHellmanTripleMessage(const EC_GROUP* Group, const EC_POINT* T1,
                             const EC_POINT* T2, const EC_POINT* T3,
                             const BIGNUM* s1, const BIGNUM* s2);
...
class DiffieHellmanTripleVerifier : public SigmaProtocolVerifier {
 public:
  DiffieHellmanTripleVerifier(const DiffieHellmanTripleCommonInput& params,
