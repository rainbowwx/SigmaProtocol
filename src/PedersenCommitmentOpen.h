...
// This protocol Prover proves that he knows openings of public Perdersen Commitment c
// Prover knows w, w2 such that c = G^w * H^w2 mod p
...
public:
    ...
    PedersenCommitmentCommonInput(const EC_GROUP* Group, const EC_POINT* g, const EC_POINT* h, const EC_POINT* Commitment, const char* HashName = "sha256")
                :Group(Group), G(g), H(h), Commitment(Commitment), SigmaProtocolCommonInput(HashName)
    {
...
public:
    ...
    PedersenCommitmentProverInput(const BIGNUM* x, const BIGNUM* r): w1(x), w2(r) {}
...
public:
    explicit PedersenCommitmentMessage(const EC_GROUP* Group): Group(Group),
                                                               T(EC_POINT_new(Group)), s1(BN_new()), s2(BN_new()){}

    PedersenCommitmentMessage(const EC_GROUP* Group, const EC_POINT* d, const BIGNUM* u, const BIGNUM* v)
...
public:
    PedersenCommitmentProver(const PedersenCommitmentCommonInput &params,
                             const PedersenCommitmentProverInput &input)
                              : params(params), input(input), Msg(params.getGroup()), r1(BN_new()), r2(BN_new()),
                              flag1(0), flag2(0) {}
...
public:
    ...
    PedersemCommitmentVerifier(const PedersenCommitmentCommonInput &params, const PedersenCommitmentMessage &Msg)
